...
func createTimeEntry(entry timeEntry) (string, error) {	
	url := fmt.Sprintf("https://api.track.toggl.com/api/v9/workspaces/%d/time_entries", workspaceId)
	
	resp, err := request(url, http.MethodPost, createTimeEntryRequest{
		Start: entry.Start,
		Stop: entry.Stop,
...
func getTimeEntriesInPast24Hours() ([]timeEntry, error) {
	...
	resp, err := request(url, http.MethodGet, getTimeEntriesRequest {
	...
	err = json.Unmarshal([]byte(resp), &timeEntries)
